...
function hasPackageJson() {
  return import_fs.default.existsSync(process.cwd() + "/package.json");
}
...
function createPrompt(title) {
  return new Promise(async (resolve, _) => {
    const status = await import_coc2.window.showPrompt(title);
    resolve(status);
  });
}
function createQuickPick(title, items) {
  return new Promise(async (resolve, _) => {
    const status = await import_coc2.window.showQuickPick(items, { title });
    resolve(status);
  });
}
...
async function executor() {
  ...
  const language = (await import_coc3.workspace.document).languageId;
  const file = (await import_coc3.workspace.document).uri;
  ...
  if (!executorSimpleMap[language]) {
    import_coc3.window.showErrorMessage(`No executor found for language: ${language}`);
    return;
  }
  const command = executorSimpleMap[language];
  ...
  if (!dynamic.some((item) => command.includes(item))) {
    ...
    return {
      type: 1,
      cmd: command.replace(/\$dir/gi, dir).replace(/\$fileNameWithoutExt/gi, outputFile).replace(/\$fileName/gi, inputFile),
      dir,
      inputFile,
      outputFile
    };
...
var RunCodeActionProvider = class {
  constructor() {
  }
  provideCodeActions() {
    const actions = [];
    if (_process) {
      actions.push(
        import_vscode_languageserver_protocol.CodeAction.create(
          ...
          import_coc4.CodeActionKind.Empty
        )
      );
    } else {
      actions.push(
...
async function activate(context) {
  ...
  if (!_channel) {
    _channel = import_coc5.window.createOutputChannel("coc-code-runner");
  }
  ...
  context.subscriptions.push(
    import_coc5.languages.registerCodeActionProvider(
      supportedLanguages.map((language) => ({ scheme: "file", language })),
      new RunCodeActionProvider(),
      "code-runner"
    ),
    import_coc5.commands.registerCommand("code-runner.stop", async () => {
      if (_channel) {
      ...
      import_coc5.window.showInformationMessage("Code execution stopped.");
    }),
    import_coc5.commands.registerCommand("code-runner.run", async () => {
      if (_channel) {
        _channel.dispose();
      }
      if (_process) {
      ...
      try {
        ...
        _channel.append("[Error] " + e?.message);
        import_coc5.window.showErrorMessage(e?.message);
        return;
      }
      if (command) {
        import_coc5.window.showInformationMessage("Running: " + command.cmd);
        _channel.appendLine("[Running] " + command.cmd);
        const startTime = /* @__PURE__ */ new Date();
        _process = (0, import_child_process.spawn)(command.cmd, [], { shell: true });
        ...
        _process.on("close", async (code) => {
          ...
          const elapsedTime = (endTime.getTime() - startTime.getTime()) / 1e3;
          _channel.appendLine("");
          _channel.appendLine(
          ...
          if (command.type == 1) {
            try {
              await (0, import_promises.rm)(command.outputFile);
            } catch {
            }
          }
          if (_process) {
