...
async function executor() {
  ...
  const language = (await import_coc2.workspace.document).languageId;
  if (!executorSimpleMap[language]) {
    import_coc2.window.showErrorMessage(`No executor found for language: ${language}`);
    return;
  }
  const command = executorSimpleMap[language];
  const file = (await import_coc2.workspace.document).uri;
...
async function activate(context) {
  ...
  context.subscriptions.push(
    import_coc4.languages.registerCodeActionProvider(
    ...
    import_coc4.commands.registerCommand("coc-code-runner.run", async () => {
      ...
      try {
        ...
        import_coc4.window.showErrorMessage(e?.message);
        return;
      }
      if (command) {
        import_coc4.window.showInformationMessage("Running: " + command.cmd);
